...
# -------------------------------------------
.PHONY: run-scopeo-agents-backend
run-scopeo-agents-backend:
	@echo "Running scopeo agents backend"
	@poetry run python -m ada_backend.main

.PHONY: run-backend-prod
run-backend-prod:
	@echo "Running Ada in production"
	@poetry run gunicorn -w 2 -k uvicorn.workers.UvicornWorker ada_backend.main:app
...
get-supabase-token:
	...
	@poetry run python -m ada_backend.scripts.get_supabase_token --username $(username) --password $(password)

# -------------------------------------------
# Database Migrations (Alembic)
# -------------------------------------------
ALEMBIC_CMD = poetry run alembic -c ada_backend/database/alembic.ini
...
db-seed:
	...
	@poetry run python -m ada_backend.database.seed_db
...
# -------------------------------------------
TRACE_ALEMBIC_CMD = poetry run alembic -c engine/trace/alembic.ini
