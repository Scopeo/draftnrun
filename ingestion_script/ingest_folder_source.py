    ...
    try:
        ...
        try:
            if len(files_info) == 0:
                ...
                created_source_id = (
                    UUID(created_source["id"])
                    if isinstance(created_source, dict) and created_source.get("id")
                    else source_id
                )
                ...
                LOGGER.info(
                    f"[EMPTY_FOLDER] Calling update_ingestion_task with status: {ingestion_task_completed.status}"
                )
                update_ingestion_task(
                    organization_id=organization_id,
