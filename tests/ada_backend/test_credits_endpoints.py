...
def create_component_version_cost_in_db(session, component_version_id: UUID, **kwargs):
    """Helper to create a component version cost in the database."""
...
def create_organization_limit_in_db(session, organization_id: UUID, year: int, month: int, limit: float):
    """Helper to create an organization limit in the database."""
    org_limit = db.OrganizationLimit(
        organization_id=organization_id,
        year=year,
        month=month,
        limit=limit,
    )
    session.add(org_limit)
    session.commit()
    session.refresh(org_limit)
    return org_limit
...
def test_get_all_organization_limits(db_session):
    ...
    try:
        ...
        existing_1 = (
            ...
            .filter_by(organization_id=org_id_1, year=test_year, month=test_month)
            .first()
        )
        if existing_1:
            db_session.delete(existing_1)

        existing_2 = (
            db_session.query(db.OrganizationLimit)
            .filter_by(organization_id=org_id_2, year=test_year, month=test_month)
            .first()
        )
        if existing_2:
            db_session.delete(existing_2)

        existing_3 = (
            db_session.query(db.OrganizationLimit)
            .filter_by(organization_id=org_id_1, year=test_year, month=test_month + 1 if test_month < 12 else 1)
            .first()
        )
        if existing_3:
            db_session.delete(existing_3)
        ...
        db_session.delete(limit_1)
        db_session.delete(limit_2)
        db_session.delete(limit_3)
        db_session.commit()
    finally:
        pass
...
def test_create_organization_limit_success(db_session, unique_year_month):
    ...
    try:
        ...
        existing = (
            db_session.query(db.OrganizationLimit)
            .filter_by(organization_id=org_id, year=unique_year, month=unique_month)
            .first()
        )
        if existing:
        ...
        org_limit = db_session.query(db.OrganizationLimit).filter_by(id=limit_id).first()
        if org_limit:
            db_session.delete(org_limit)
            db_session.commit()
    finally:
        pass
...
def test_delete_organization_limit_success(db_session):
    ...
    try:
        ...
        deleted_limit = db_session.query(db.OrganizationLimit).filter_by(id=limit_id).first()
...
def test_get_all_organization_limits_with_filters(db_session):
    ...
    try:
        ...
        for month in [month_1, month_2, month_3]:
            existing = (
                db_session.query(db.OrganizationLimit)
                .filter_by(organization_id=org_id, year=test_year, month=month)
                .first()
            )
            if existing:
                db_session.delete(existing)
...
def test_upsert_component_version_cost_create(db_session, ensure_component_version):
    ...
    try:
        ...
        component_cost = (
            db_session.query(db.ComponentCost).filter_by(component_version_id=component_version_id).first()
        )
        if component_cost:
            db_session.delete(component_cost)
            db_session.commit()
    finally:
        pass
...
def test_delete_component_version_cost_success(db_session, ensure_component_version):
    ...
    try:
        ...
        deleted_cost = db_session.query(db.ComponentCost).filter_by(component_version_id=component_version_id).first()
