...
def _setup_graph_with_component(session):
    project = db.WorkflowProject(id=uuid.uuid4(), name="TestProject", organization_id=ORG_ID)
    session.add(project)
    session.flush()
    ...
    binding = db.ProjectEnvironmentBinding(
        id=uuid.uuid4(), project_id=project.id, graph_runner_id=gr.id, environment=db.EnvType.DRAFT,
    )
    session.add(binding)
    session.flush()
...
class TestPinOwnershipValidation:
    ...
    def test_pin_with_wrong_project_raises(self):
        with get_db_session() as session:
            ci, gr, ipi, project = _setup_graph_with_component(session)
            prompt = _create_prompt(session, name=f"Pin-{uuid.uuid4().hex[:8]}")
            detail = get_prompt_detail_service(session, prompt.id, organization_id=ORG_ID)
            ...
            with pytest.raises(GraphNotBoundToProjectError):
                pin_prompt_to_port_service(
                    session, ci.id, "system_prompt", version_id, gr.id, wrong_project_id,
                )

    def test_unpin_with_wrong_project_raises(self):
        with get_db_session() as session:
            ci, gr, ipi, project = _setup_graph_with_component(session)
