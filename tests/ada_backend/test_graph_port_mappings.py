...
def test_get_put_roundtrip_port_mappings_migration():
    ...
    with get_db_session() as session:
        ...
        data = get_graph_service(session, project_id, graph_runner_id).model_dump()
        ...
        assert str(pm["source_instance_id"]) == src_instance_id
        assert str(pm["target_instance_id"]) == dst_instance_id
        ...
        _run_update_graph(session, graph_runner_id, project_id, roundtrip_payload)

        # GET again; port_mappings should remain the same (idempotent)
        data2 = get_graph_service(session, project_id, graph_runner_id).model_dump()
...
def test_deploy_graph_copies_port_mappings():
    ...
    with get_db_session() as session:
        ...
        original_data = get_graph_service(session, project_id, graph_runner_id).model_dump()
        ...
        assert deploy_data.draft_graph_runner_id
        ...
        new_data = get_graph_service(session, project_id, new_draft_graph_id).model_dump()
        ...
        assert str(new_pm["source_instance_id"]) != src_instance_id
        assert str(new_pm["target_instance_id"]) != dst_instance_id

        # Verify the production graph also has the port mappings
        prod_data = get_graph_service(session, project_id, graph_runner_id).model_dump()
