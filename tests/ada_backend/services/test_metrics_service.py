...
def test_get_trace_cost_kpis():
    ...
    with get_db_session() as session:
        ...
        result_both_with_call_type = get_trace_cost_kpis([project1, project2], duration_days=1, call_type=CallType.API)
        ...
        llm_cost_to_delete = session.query(LLMCost).filter(LLMCost.llm_model_id == model_id).first()
        session.query(LLMCost).filter(LLMCost.llm_model_id == model_id).delete(synchronize_session=False)

        session.query(Cost).filter(Cost.id == llm_cost_to_delete.id).delete(synchronize_session=False)
        session.query(LLMModel).filter(LLMModel.id == model_id).delete(synchronize_session=False)
