...
class TestGetWorkflowsByOrganizationService:
    ...
    def test_returns_list_of_project_schemas(
        ...
        """
        result = get_workflows_by_organization_service(
    ...
    def test_returns_lightweight_schema_without_graph_runners(
        ...
        for project in result:
            ...
            assert not hasattr(project, 'created_at')
    ...
    def test_tracks_user_analytics_when_user_id_provided(
    ...
    def test_skips_analytics_when_no_user_id(
        ...
        with patch('ada_backend.services.project_service.track_user_get_project_list') as mock_track:
            get_workflows_by_organization_service(
                db_session, test_organization, user_id=None
            )

            # Should not call tracking
            mock_track.assert_not_called()

    def test_returns_correct_project_data(
        self, db_session: Session, test_projects_with_versions, test_organization
    ):
        """
        ...
        matching_result = next(
            (p for p in result if p.project_id == test_project.id),
            None
        )
...
class TestGetProjectService:
    ...
    def test_returns_project_with_graph_runners_schema(
        self, db_session: Session, test_projects_with_versions
    ):
        """
    ...
    def test_includes_all_graph_runners_with_environments(
        self, db_session: Session, test_projects_with_versions
    ):
        """
        ...
        gr_data = {
            gr.graph_runner_id: gr.env
            for gr in result.graph_runners
        }
    ...
    def test_includes_tag_versions(
        self, db_session: Session, test_projects_with_versions
    ):
        """
        ...
        version_map = {
            gr.graph_runner_id: gr.tag_version
            for gr in result.graph_runners
        }
    ...
    def test_includes_complete_project_metadata(
        self, db_session: Session, test_projects_with_versions
    ):
        """
    ...
    def test_raises_error_for_nonexistent_project(
    ...
    def test_graph_runners_ordered_correctly(
        self, db_session: Session, test_organization
    ):
        """
        ...
        # Create graph runners in specific order
        gr_ids = []
        for i in range(3):
            gr = db.GraphRunner(id=uuid.uuid4(), tag_version=f"v{i}")
            db_session.add(gr)
            db_session.flush()
        ...
        # Graph runners should be in creation order
        result_gr_ids = [gr.graph_runner_id for gr in result.graph_runners]
        assert result_gr_ids == gr_ids
...
class TestServiceIntegration:
    ...
    def test_list_service_does_not_include_graph_runner_data(
        ...
        """
        list_result = get_workflows_by_organization_service(
            db_session, test_organization
        )
    ...
    def test_detail_service_includes_graph_runner_data(
        self, db_session: Session, test_projects_with_versions
    ):
        """
    ...
    def test_list_then_detail_consistency(
        self, db_session: Session, test_projects_with_versions, test_organization
    ):
        """
