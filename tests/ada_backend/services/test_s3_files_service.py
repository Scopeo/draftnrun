...
def test_generate_file_download_url_requires_organization_prefix(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    response = s3_files_service.generate_file_download_url_service(
...
def test_generate_file_download_url_rejects_other_organization_key() -> None:
    with pytest.raises(s3_files_service.FileDownloadKeyValidationError, match="does not belong"):
        s3_files_service.generate_file_download_url_service(
            organization_id=uuid4(),
