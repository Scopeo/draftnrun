...
class TestSyncInputPortFieldExpressionsPromptPin:
    ...
    @patch("ada_backend.services.graph.component_instance_v2_service.get_input_port_instances_for_component_instance")
    def test_updates_existing_port_with_prompt_version_id(
        self, mock_get_ports, mock_update_fe, mock_update_ipi, session
    ):
        version_id = uuid4()
        ...
        mock_update_fe.assert_called_once_with(
            session, existing_port.field_expression_id, {"type": "literal", "value": "Updated"}
        )
