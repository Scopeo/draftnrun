...
def test_graph_setup():
    ...
    with get_db_session() as session:
        ...
        for obj in [edge1, edge2]:
            session.delete(obj)
        session.flush()
        for obj in [upstream_node, target_node, downstream_node]:
            session.delete(obj)
        session.flush()
        for obj in [upstream_instance, target_instance, downstream_instance]:
            session.delete(obj)
        session.flush()
        session.delete(output_port)
        session.delete(component_version)
        session.delete(component)
        session.delete(env_binding)
        session.delete(graph_runner)
...
def test_no_upstream_instances_returns_empty():
