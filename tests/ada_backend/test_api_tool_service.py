...
Tests business logic and ensures proper separation of concerns.
"""
...
def test_create_tool_creates_unique_component(clean_test_session, sample_tool_payload):
    ...
    assert result1.name == sample_tool_payload.tool_display_name
    ...
    matching_tools = [t for t in tools_list.tools if t.name == sample_tool_payload.tool_display_name]
...
def test_list_tools_excludes_base_component(clean_test_session, sample_tool_payload):
    ...
    api_call_tools = [t for t in tools_list.tools if t.name == "API Call"]
...
def test_update_one_tool_doesnt_affect_others(clean_test_session, sample_tool_payload):
    ...
    # Get details of second tool before update
