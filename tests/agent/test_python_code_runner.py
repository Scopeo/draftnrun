...
"""
    ...
    result = asyncio.run(e2b_tool._run_without_io_trace(inputs, ctx={}))
    ...
    # Verify the image path is a string
...
"""
    ...
    # Verify both image paths are strings
...
def test_run_without_io_trace_simple_code(e2b_tool, e2b_api_key):
    ...
    assert result.execution_result["error"] == execution_data["error"]
    assert result.execution_result["stdout"] == execution_data["stdout"]
    assert result.execution_result["stderr"] == execution_data["stderr"]
