...
def test_split_method():
    ...
    input_payload = AgentPayload(messages=[ChatMessage(role="user", content="word1 word2 word3 word4 word5 word6")])
    chunks = chunk_processor._split(input_payload)

    assert len(chunks) == 3
    assert all(isinstance(chunk, AgentPayload) for chunk in chunks)
...
def test_chunk_processor_end_to_end(mock_agent_calls, mock_get_tracing_span, simple_graph_runner, mock_trace_manager):
    ...
    # Test processing
    input_payload = AgentPayload(messages=[ChatMessage(role="user", content="hello world test message")])
...
def test_chunk_processor_parameters(mock_trace_manager, simple_graph_runner):
    ...
    # Test different n_chunks values
    for n_chunks in [1, 3, 5]:
        chunk_processor = ChunkProcessor(
            ...
            component_attributes=ComponentAttributes(
                component_instance_id=uuid.uuid4(),
                component_instance_name=f"test_{n_chunks}",
            ),
            n_chunks=n_chunks,
            join_char=" | ",
        )

        assert chunk_processor._n_chunks == n_chunks
