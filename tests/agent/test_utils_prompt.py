...
class TestFillPromptTemplate:
    ...
    def test_single_braces_are_escaped_not_replaced(self):
        """Test that single braces are preserved and not treated as template variables."""
        template = "This has {single} braces but {{double}} braces work"
        variables = {"double": "replaced"}
        result = fill_prompt_template(template, component_name="test", variables=variables)
