...
def test_structured_output_in_function_call_async(
    ...
    # Test 2: Regular tools called (no structured output)
    mock_regular_tool_call = MagicMock()
    mock_regular_tool_call.id = "1"
    mock_regular_tool_call.function.name = "test_tool"
    mock_regular_tool_call.function.arguments = json.dumps({"test_property": "value"})

    mock_message_with_tools = MagicMock()
    mock_message_with_tools.content = None
    mock_message_with_tools.tool_calls = [mock_regular_tool_call]
    mock_message_with_tools.model_dump = lambda: {
        "role": "assistant",
        "content": None,
        "tool_calls": [
            {"id": "1", "function": {"name": "test_tool", "arguments": json.dumps({"test_property": "value"})}}
        ],
    }

    mock_llm_service.function_call_async = AsyncMock(
        return_value=MagicMock(choices=[MagicMock(message=mock_message_with_tools)])
    )

    # Mock tool execution
    with patch.object(react_agent, "_process_tool_calls") as mock_process:
        mock_process.return_value = (
            {"1": AgentPayload(messages=[ChatMessage(role="assistant", content="Tool result")], is_final=True)},
            [
                {
                    "id": "1",
                    "function": {"name": "test_tool", "arguments": json.dumps({"test_property": "value"})},
                    "type": "function",
                }
            ],
        )
        ...
        output = react_agent.run_sync(agent_input)
        assert output.last_message.content == "Tool result"
        assert output.is_final

    # Test 3: Structured output tool called with direct data
    ...
    mock_structured_tool_call.function.arguments = json.dumps({"answer": "Final answer", "is_final": True})
    ...
    mock_llm_service.function_call_async = AsyncMock(
        return_value=MagicMock(choices=[MagicMock(message=mock_message_structured)])
    )
    ...
    # Test 4: JSON validation - structured output with schema format (properties wrapper)
    mock_schema_tool_call = MagicMock()
    mock_schema_tool_call.id = "3"
    mock_schema_tool_call.function.name = "chat_formatting_output_tool"
    # Simulate model returning schema format instead of direct data
    mock_schema_tool_call.function.arguments = {"properties": {"answer": "Schema wrapped answer", "is_final": False}}

    mock_message_schema = MagicMock()
    mock_message_schema.content = None
    mock_message_schema.tool_calls = [mock_schema_tool_call]
    mock_message_schema.model_dump = lambda: {
        "role": "assistant",
        "content": None,
        "tool_calls": [
            {
                ...
                "function": {
                    "name": "chat_formatting_output_tool",
                    "arguments": {"properties": {"answer": "Schema wrapped answer", "is_final": False}},
                },
            }
        ],
    }
    ...
    mock_llm_service.function_call_async = AsyncMock(
        return_value=MagicMock(choices=[MagicMock(message=mock_message_schema)])
    )

    output = react_agent.run_sync(agent_input)
    # Should return the full payload including the "properties" wrapper
    assert output.last_message.content == json.dumps(
        {"properties": {"answer": "Schema wrapped answer", "is_final": False}}
    )
    assert output.is_final

    # Test 5: No tools called - backup method with constrained_complete_with_json_schema_async
    mock_message_no_tools = MagicMock()
    mock_message_no_tools.content = "Regular response"
    mock_message_no_tools.tool_calls = []
    mock_message_no_tools.model_dump = lambda: {"role": "assistant", "content": "Regular response", "tool_calls": []}

    # Mock the backup constrained method
    with patch.object(mock_llm_service, "constrained_complete_with_json_schema_async") as mock_constrained:
        mock_constrained.return_value = json.dumps({"answer": "Backup answer", "is_final": True})

    mock_llm_service.function_call_async = AsyncMock(
        return_value=MagicMock(choices=[MagicMock(message=mock_message_no_tools)])
    )

    output = react_agent.run_sync(agent_input)
    assert output.last_message.content == json.dumps({"answer": "Backup answer", "is_final": True})
    assert output.is_final
