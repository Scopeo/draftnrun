...
class TestGraphRunnerFormulas:
    def test_formula_derived_dependency_orders_execution(self):
    ...
    def test_pure_ref_formula_is_ignored_in_presence_of_mapping(self):
        tm = TraceManager(project_name="test")
        set_tracing_span(project_id="test_proj", organization_id="org", organization_llm_providers=["mock"])  # metrics
        ...
        # Pure ref formula to a different source; should be ignored at runtime, mapping wins
        formulas = [
            {
                "target_instance_id": "B",
                "field_name": "input",
                "formula_json": {"type": "ref", "instance": "C", "port": "output"},
            }
        ]
    ...
    def test_non_ref_formula_overrides_mapping(self):
        tm = TraceManager(project_name="test")
        set_tracing_span(project_id="test_proj", organization_id="org", organization_llm_providers=["mock"])  # metrics
        ...
        formulas = [
            {"target_instance_id": "B", "field_name": "input", "formula_json": {"type": "literal", "value": "OVR"}}
        ]
    ...
    def test_multi_ref_concat_merges_sources(self):
        ...
        # Provide mapping as well; non-ref formula overrides mapping value
        mappings = [
            {
                "source_instance_id": "A",
                "source_port_name": "output",
    ...
    def test_cycle_detected_via_formula_refs(self):
        tm = TraceManager(project_name="test")
        g = nx.DiGraph()
        g.add_nodes_from(["A", "B"])
        ...
        formulas = [
            {
                "target_instance_id": "B",
                "field_name": "input",
                "formula_json": {"type": "ref", "instance": "A", "port": "output"},
            },
            {
                "target_instance_id": "A",
                "field_name": "input",
                "formula_json": {"type": "ref", "instance": "B", "port": "output"},
            },
        ]

        try:
            GraphRunner(graph=g, runnables=runnables, start_nodes=["A"], trace_manager=tm, formulas=formulas)
            assert False, "Expected cycle detection"
        except ValueError as e:
            assert "cycles" in str(e).lower()

    def test_self_loop_detected_via_formula(self):
        tm = TraceManager(project_name="test")
        g = nx.DiGraph()
        g.add_nodes_from(["A"])
        ...
        formulas = [
            {
                ...
                "formula_json": {"type": "literal", "value": "x"},
            }
        ]
        try:
            GraphRunner(graph=g, runnables=runnables, start_nodes=["A"], trace_manager=tm, formulas=formulas)
    ...
    def test_invalid_target_component_validation(self):
        ...
        formulas = [
            {"target_instance_id": "C", "field_name": "input", "formula_json": {"type": "literal", "value": "x"}}
        ]
        try:
            GraphRunner(graph=g, runnables=runnables, start_nodes=["A"], trace_manager=tm, formulas=formulas)
    ...
    def test_literal_formula_coerces_to_int(self):
        tm = TraceManager(project_name="test")
        set_tracing_span(project_id="test_proj", organization_id="org", organization_llm_providers=["mock"])  # metrics
        ...
        formulas = [
            {"target_instance_id": "B", "field_name": "input", "formula_json": {"type": "literal", "value": "42"}}
        ]

        gr = GraphRunner(graph=g, runnables=runnables, start_nodes=["B"], trace_manager=tm, formulas=formulas)
        result = asyncio.run(gr.run({"input": "ignored"}))
        assert result.messages[0].content == "42"

    def test_formula_string_coerces_to_list_chatmessage(self):
        tm = TraceManager(project_name="test")
        set_tracing_span(project_id="test_proj", organization_id="org", organization_llm_providers=["mock"])  # metrics
        ...
        formulas = [
            {
                ...
                "field_name": "messages",
                "formula_json": {"type": "literal", "value": "hello"},
            }
        ]

        gr = GraphRunner(graph=g, runnables=runnables, start_nodes=["S"], trace_manager=tm, formulas=formulas)
        result = asyncio.run(gr.run({"input": "seed"}))
        assert result.messages[0].content == "msgs:1 last[hello]"

    def test_dual_input_with_mapping_and_formula(self):
        tm = TraceManager(project_name="test")
        set_tracing_span(project_id="test_proj", organization_id="org", organization_llm_providers=["mock"])  # metrics
        ...
        formulas = [
            {"target_instance_id": "JOIN", "field_name": "b", "formula_json": {"type": "literal", "value": "X"}}
        ]
    ...
    def test_dual_input_formula_overrides_mapped_field(self):
        tm = TraceManager(project_name="test")
        set_tracing_span(project_id="test_proj", organization_id="org", organization_llm_providers=["mock"])  # metrics
        ...
        # Override 'a' via formula
        formulas = [
            {"target_instance_id": "JOIN", "field_name": "a", "formula_json": {"type": "literal", "value": "OVR"}}
        ]
    ...
    def test_formulas_for_unmigrated_target_raise(self):
        tm = TraceManager(project_name="test")
        g = nx.DiGraph()
        ...
        formulas = [
            {"target_instance_id": "U", "field_name": "input", "formula_json": {"type": "literal", "value": "x"}}
        ]

        try:
            GraphRunner(graph=g, runnables=runnables, start_nodes=["U"], trace_manager=tm, formulas=formulas)
            assert False, "Expected formulas not supported for unmigrated target"
        except ValueError as e:
            assert "formulas are not supported" in str(e).lower()
    ...
    def test_mixed_concat_with_int_ref(self):
        ...
        gr = GraphRunner(graph=g, runnables=runnables, start_nodes=["A", "B"], trace_manager=tm, formulas=formulas)
...
class TestGraphRunnerComplexFormulas:
    """Mixed mappings and formulas across a diamond shape."""
    ...
    def _build_diamond_graph(self):
        ...
        return tm, g, runnables, mappings, formulas

    def test_diamond_graph_execution(self):
        tm, g, runnables, mappings, formulas = self._build_diamond_graph()
        gr = GraphRunner(
            graph=g,
            runnables=runnables,
            start_nodes=["A"],
