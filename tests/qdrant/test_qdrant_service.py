...
def test_merge_qdrant_filters():
    ...
    if asyncio.run(qdrant_service.collection_exists_async(TEST_COLLECTION_NAME_MERGE)):
        asyncio.run(qdrant_service.delete_collection_async(TEST_COLLECTION_NAME_MERGE))

    asyncio.run(qdrant_service.create_collection_async(collection_name=TEST_COLLECTION_NAME_MERGE))
    ...
    asyncio.run(qdrant_service.add_chunks_async(list_chunks=chunks, collection_name=TEST_COLLECTION_NAME_MERGE))
    ...
    results_filter_1 = asyncio.run(
        qdrant_service.retrieve_similar_chunks_async(
            query_text="chunk",
