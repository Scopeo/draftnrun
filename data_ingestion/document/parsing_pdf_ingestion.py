    ...
    try:
        ...
        if isinstance(content_to_process, bytes):
            ...
            try:
                ...
                LOGGER.error(f"Error parsing PDF {document.title} without LLM: {e}", exc_info=True)
                raise Exception(f"Error parsing PDF {document.title} without LLM") from e
            finally:
                if temp_path.exists():
                    os.unlink(temp_path)
        else:
        ...
        LOGGER.error(f"Error processing PDF {document.title} without LLM: {e}", exc_info=True)
        raise Exception(f"Error processing PDF {document.title} without LLM") from e
