...
serve(async req => {
  ...
  try {
    ...
    const { data: existingUser, error: userCheckError } = await supabaseAdmin.rpc('get_user_by_email', { email })
    ...
    if (existingUser) {
      ...
      if (fullUserData) {
        if (fullUserData.email_confirmed_at) {
          ...
          console.log('🔄 About to resend verification email to existing user...')
          try {
            ...
            console.log('⚠️ Process will continue despite email failure')
          }
          ...
          return new Response(
            JSON.stringify({
              ...
              message:
                'Account exists but email not verified. A new verification email has been sent to your email address.',
            }),
            {
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
              status: 200,
    ...
    console.log(
      `🔧 Created custom verification URL for ${isDevelopment ? 'development' : 'production'}:`,
      verificationUrl
    )
    ...
    try {
      ...
      // Don't throw here - let user creation succeed even if email fails
      console.log('⚠️ User creation will continue despite email failure')
    }

    // Store user info for org creation after email verification
    const { error: pendingOrgError } = await supabaseAdmin.from('pending_user_orgs').insert({
    ...
    console.log(
      `User ${newUser.user.id} created, verification email sent to ${email} (${isDevelopment ? 'DEV' : 'PROD'})`
    )
