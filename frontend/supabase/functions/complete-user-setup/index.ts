...
serve(async req => {
  ...
  try {
    ...
    // Set initial monthly credit limit
    try {
      const backendUrl = Deno.env.get('VITE_SCOPEO_API_URL')
      const defaultCreditLimit = parseInt(Deno.env.get('DEFAULT_CREDIT_LIMIT') || '2000', 10)
      ...
      if (!backendUrl) {
        ...
        const response = await fetch(`${backendUrl}/organizations/${newOrg.id}/organization-limits`, {
          ...
          headers: {
            ...
            'X-Admin-API-Key': `${Deno.env.get('ADMIN_KEY')}`,
          },
          body: JSON.stringify({
        ...
        if (!response.ok) {
          ...
          console.error('Failed to set initial credit limit:', errorText)
          console.warn('Organization created but initial credit limit could not be set')
        } else {
          console.log(`✅ Initial credit limit of ${defaultCreditLimit} set successfully`)
        }
      }
    } catch (creditError) {
      console.error('Error setting initial credit limit:', creditError)
      console.warn('Organization created but initial credit limit could not be set due to error')
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        session, // Include session data for frontend authentication
