  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const superAdminsWithDetails = superAdmins.map(sa => {
        ...
        console.log(
          `[DEBUG] Mapping super admin ${sa.user_id}: ${user ? `found user ${user.email}` : 'USER NOT FOUND'}`
        )
        return {
          ...sa,
      ...
      console.log(
        `[DEBUG] Final super admins with details:`,
        superAdminsWithDetails.map(sa => ({ id: sa.user_id, email: sa.email }))
      )

      return new Response(JSON.stringify({ superAdmins: superAdminsWithDetails }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    if (req.method === 'POST') {
      ...
      return new Response(JSON.stringify({ message: 'Super admin added successfully' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    if (req.method === 'DELETE') {
      ...
      if (userId === user.id) {
        ...
        return new Response(JSON.stringify({ error: 'Cannot remove yourself as super admin' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
      ...
      const { error: deleteError } = await supabase.from('super_admins').delete().eq('user_id', userId)
      ...
      return new Response(JSON.stringify({ message: 'Super admin removed successfully' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
