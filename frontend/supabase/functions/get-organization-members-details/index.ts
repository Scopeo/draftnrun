...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(
      `[get-organization-members-details] Membership check for user ${user.id} in org ${orgId} - Count: ${memberCount}, Error: ${memberCheckError ? memberCheckError.message : 'No error'}`
    )

    if (memberCheckError) {
      console.error(
        `[get-organization-members-details] Error during organization membership check: ${memberCheckError.message}`
      )
    ...
    if (memberCount === null || memberCount === 0) {
      console.warn(
        `[get-organization-members-details] Access Denied: User ${user.id} attempted to access org ${orgId}. Membership count: ${memberCount}.`
      )
    ...
    console.log(
      `[get-organization-members-details] Membership confirmed for user ${user.id} in org ${orgId}. Proceeding to fetch members.`
    )
