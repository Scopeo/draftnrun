...
def make_inference_request(
    project_id: str, api_key: str, input_data: Dict[str, Any], timeout: int = 600
) -> Dict[str, Any]:
    """
    ...
    try:
        response = requests.post(url=url, headers=headers, json=input_data, timeout=timeout)

        response.raise_for_status()
        result = response.json()

        LOGGER.info(f"Successful inference request for project {project_id}")
        return {"status": "SUCCESS", "result": result, "status_code": response.status_code}

    except requests.exceptions.HTTPError as e:
        error_msg = f"HTTP error {e.response.status_code}: {e.response.text}"
        LOGGER.error(f"HTTP error in inference request: {error_msg}")
        return {"status": "HTTP_ERROR", "error": error_msg, "status_code": e.response.status_code}
    except requests.exceptions.Timeout:
        error_msg = f"Request timeout after {timeout} seconds"
        LOGGER.error(f"Timeout in inference request: {error_msg}")
        return {"status": "TIMEOUT_ERROR", "error": error_msg}
    except requests.exceptions.RequestException as e:
        error_msg = f"Request error: {str(e)}"
        LOGGER.error(f"Request error in inference request: {error_msg}", exc_info=True)
        return {"status": "REQUEST_ERROR", "error": error_msg}
...
def execute_scheduled_workflow(self, project_id: str, scheduled_workflow_uuid: str, type: str, args: str, **kwargs):
    ...
    try:
        ...
        input_data = {
            "messages": [
                {"role": "user", "content": f"Scheduled execution triggered at {start_time.isoformat()} UTC"}
            ],
        ...
        if result["status"] == "SUCCESS":
            LOGGER.info(
                f"Scheduled workflow execution completed successfully: "
                f"task={task_id}, project={project_id}, time={execution_time}s"
            )
            return {
                "status": "SUCCESS",
                "task_id": task_id,
