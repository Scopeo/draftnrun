...
Unified Mail Sender remains an exactly-one-provider component at runtime. The studio's exclusive OAuth group should let users switch between provider selectors without clearing the saved connection; it should clear the other provider only when a new non-null connection is chosen.
