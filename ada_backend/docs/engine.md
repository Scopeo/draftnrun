...
Agent LLM message as one-shot file payloads, and `artifacts.attached_files` is cleared after that request is built. Direct
LLM file payloads use the normalized run-relative path as `filename`, preserving subdirectory context for files with the
same basename. Missing, oversized, or unreadable PDFs are skipped per file so the remaining attachments can still be sent.
If the retained message window has no user-role message to carry the file payloads, the agent adds a short fallback user
message for that next LLM call.
...
HubSpot MCP `notes_upsert_for_contact` and `tasks_create` accept `properties.hs_timestamp` as optional. When omitted or empty, the tool sends the current UTC timestamp to HubSpot in ISO-8601 `Z` format before creating or updating the engagement.
