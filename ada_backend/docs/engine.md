...
Mail sender components (`Gmail Sender`, `Gmail Neverdrop`, `Outlook Sender`, and unified `Mail Sender`) accept `email_attachments` as a list of string paths/URLs or a list of objects shaped like `{"url": "...", "filename": "..."}`. String URL attachments keep the existing URL basename behavior. Object attachments download from `url` and attach using the provided `filename`, which lets workflows control the displayed file name in Gmail MIME messages and Microsoft Graph payloads.
