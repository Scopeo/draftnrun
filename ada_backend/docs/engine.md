...
Integration-backed catalog entries can share a runtime class while using distinct OAuth provider keys. For example, Gmail Neverdrop reuses `GmailSenderV2` with `google-mail-neverdrop`, Google Calendar Neverdrop reuses `GoogleCalendarMCPTool` with `google-calendar-neverdrop`, and Google Contacts Neverdrop uses the read-only `GoogleContactsMCPTool` with `google-contact-neverdrop`. Gmail Neverdrop must remain send-only: drafts are disabled, recipients are required, and no `save_as_draft` runtime argument is exposed. Neverdrop-branded Google integration tools should remain agent-available (`is_agent=True`) and function-callable. Google API discovery services used from async MCP clients should be built inside each `asyncio.to_thread` call because the google-api-python-client service/HTTP stack is not thread-safe across worker threads.
