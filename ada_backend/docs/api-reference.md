...
1. **Init** — `POST .../multipart/init` with `{ filename, content_type }`. Returns `{ upload_id, key }`.
2. **Presign parts** — `POST .../multipart/presign-parts` with `{ key, upload_id, part_count }`. Returns presigned URLs for each part.
