...
class MigrationQueries:
    ...
    CORE_DATA_QUERIES = {
        'projects': """
            ...
            SELECT id, name, type, description, %s, created_at, updated_at
        ...
        'workflow_projects': """
            ...
            SELECT id FROM projects WHERE type = 'workflow' AND organization_id = %s
        """,
        
        'agent_projects': """
            INSERT INTO agent_projects (id)
            SELECT id FROM projects WHERE type = 'agent' AND organization_id = %s
        """
    }
