...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler("migrate_qa_projects.log")],
)
...
class QAMigrationOrchestrator:
        ...
        try:
            ...
            for table in cleanup_order:
                ...
                remaining_count = self.db_manager.get_row_count(
                    table,
                    (
                        f"id IN (SELECT id FROM projects WHERE organization_id = '{self.org_id}')"
                        if table == "projects"
                        else None
                    ),
                )
            ...
            self.db_manager.execute_preprod_query(
                MigrationQueries.CLEANUP_QUERIES["projects"], (self.org_id,), fetch=False
            )
        ...
        try:
            for table, query in queries.items():
                ...
                count = self.db_manager.get_row_count(
                    ...
                    (
                        f"id IN (SELECT id FROM projects WHERE organization_id = '{self.org_id}')"
                        if table in ["projects", "workflow_projects", "agent_projects"]
                        else None
                    ),
                )
    ...
    def execute_core_data_migration(self) -> bool:
        ...
        return self.execute_migration_step("Core Data Migration", MigrationQueries.CORE_DATA_QUERIES)

    def execute_graph_data_migration(self) -> bool:
        """Execute graph data migration (graph runners, component instances, bindings)."""
        return self.execute_migration_step("Graph Data Migration", MigrationQueries.GRAPH_DATA_QUERIES)

    def execute_graph_structure_migration(self) -> bool:
        """Execute graph structure migration (nodes and edges)."""
        return self.execute_migration_step("Graph Structure Migration", MigrationQueries.GRAPH_STRUCTURE_QUERIES)

    def execute_configuration_migration(self) -> bool:
        """Execute configuration migration (parameters, mappings, sub inputs)."""
        return self.execute_migration_step("Configuration Migration", MigrationQueries.CONFIGURATION_QUERIES)
