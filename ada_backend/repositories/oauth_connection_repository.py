...
def count_connections_by_provider(
    session: Session,
    organization_id: UUID,
    provider_config_key: str,
) -> int:
    """Count the number of connections for a given provider and organization.
    It doesn't filter active connections only, so the count it's incremental.
    """
    return (
        session.query(db.OAuthConnection)
        .filter(
            db.OAuthConnection.organization_id == organization_id,
            db.OAuthConnection.provider_config_key == provider_config_key,
        )
        .count()
    )
