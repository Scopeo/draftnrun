...
def get_latest_version_numbers_batch(session: Session, prompt_ids: list[UUID]) -> dict[UUID, int]:
    if not prompt_ids:
        return {}
    rows = (
        session.query(db.PromptVersion.prompt_id, func.max(db.PromptVersion.version_number))
        .filter(db.PromptVersion.prompt_id.in_(prompt_ids))
        .group_by(db.PromptVersion.prompt_id)
        .all()
    )
    return {pid: vn for pid, vn in rows}
