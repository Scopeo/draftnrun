...
def upgrade() -> None:
    ...
    #
    # NOTE: we read from the field_expressions created in step 2b, NOT from basic_parameters
    # (which step 2b already deleted in the same transaction — reading from them here would
        ...
        sa.text(f"""
            WITH source AS (
                SELECT
                    ...
                    (fe.expression_json->>'value')::jsonb AS schema_json
                ...
                WHERE pi.name = 'payload_schema'
                  ...
                  AND (fe.expression_json->>'value') IS NOT NULL
            ),
            keys AS (
                SELECT
                    component_instance_id,
