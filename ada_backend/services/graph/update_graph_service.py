...
def _calculate_graph_hash(graph_project: GraphUpdateSchema) -> str:
    graph_dict = graph_project.model_dump(mode="json", exclude={"last_edited_time"})
...
async def update_graph_service(
    ...
    auto_generated_canonical_expressions = _ensure_canonical_expressions_for_edges(
...
def _ensure_canonical_expressions_for_edges(
    ...
    """Auto-generate canonical RefNode field expressions for edges that have no explicit wiring.

    For each edge whose target canonical input port has no incoming field expression,
    creates an :class:`AutoGeneratedCanonicalExpression` so the wiring is visible
    (and editable) in the frontend.

    Returns a list of :class:`AutoGeneratedCanonicalExpression`. The caller must merge
    these into ``incoming_field_expressions_by_instance`` to prevent the cleanup pass
    from deleting them.
    """
    actual_edges: set[tuple[UUID, UUID]] = {(edge.origin, edge.destination) for edge in graph_project.edges}
    unmapped_edges = actual_edges
