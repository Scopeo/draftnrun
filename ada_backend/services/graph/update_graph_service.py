...
from ada_backend.schemas.pipeline.graph_schema import AutoGeneratedFieldExpression, GraphUpdateResponse, GraphUpdateSchema
...
from engine.field_expressions.serializer import from_json as expr_from_json, to_json as expr_to_json
...
async def update_graph_service(
    ...
    for target_id, field_name, expression_json in auto_generated_canonical_expressions:
        ...
        if existing_port_id:
            ...
            db_port_instances_by_instance[target_id][field_name] = expr.id
    ...
    for target_id, field_name, _ in auto_generated_canonical_expressions:
        incoming_field_expressions_by_instance[target_id].add(field_name)
    for instance in graph_project.component_instances:
...
def _ensure_port_mappings_for_edges(  # TODO(port-mapping-removal): delete when port mappings are removed
    ...
    Returns a list of ``(target_instance_id, field_name, expression_json)``
    tuples for auto-generated canonical field expressions.  The caller must
    merge these into ``incoming_field_expressions_by_instance`` to prevent
    the cleanup pass from deleting them.
    """
    ...
    auto_generated_expressions: list[tuple[UUID, str, dict]] = []
    ...
    for source_instance_id, target_instance_id in unmapped_edges:
        ...
        if (target_instance_id, target_port_name) not in incoming_fe_targets:
            ...
            auto_generated_expressions.append((target_instance_id, target_port_name, expression_json))
