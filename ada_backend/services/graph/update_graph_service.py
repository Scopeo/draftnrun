...
class AutoGeneratedCanonicalExpression:
    """An auto-generated RefNode field expression for a canonical input port."""
...
async def update_graph_service(
    ...
    auto_generated_canonical_expressions = _ensure_port_mappings_for_edges(
    ...
    # so that user-provided expressions (from input_port_instances or parameters)
    # can still take priority.
    for expr_info in auto_generated_canonical_expressions:
        target_id = expr_info.target_instance_id
        field_name = expr_info.field_name
        expression_json = expr_info.expression_json
        existing_port_id = db_port_instances_by_instance[target_id].get(field_name)
        if existing_port_id:
    ...
    # Mark auto-generated canonical expressions as "incoming" so the cleanup
    # pass does not delete them.
    for expr in auto_generated_canonical_expressions:
        incoming_field_expressions_by_instance[expr.target_instance_id].add(expr.field_name)
    for instance in graph_project.component_instances:
        if instance.id not in incoming_field_expressions_by_instance:
    ...
    auto_expr_schemas: list[AutoGeneratedFieldExpression] = []
    for expr_info in auto_generated_canonical_expressions:
        expression_json = expr_info.expression_json
        try:
            ...
            text = None
        auto_expr_schemas.append(
            AutoGeneratedFieldExpression(
                component_instance_id=expr_info.target_instance_id,
                field_name=expr_info.field_name,
    ...
    return GraphUpdateResponse(
        ...
        playground_input_schema=playground_input_schema,
        playground_field_types=playground_field_types,
        auto_generated_field_expressions=auto_expr_schemas,
    )
...
def _ensure_port_mappings_for_edges(  # TODO(port-mapping-removal): delete when port mappings are removed
    ...
    graph_runner_id: UUID,
    graph_project: GraphUpdateSchema,
    input_params_by_instance: dict[UUID, list] | None = None,
) -> list[AutoGeneratedCanonicalExpression]:
    """Ensure explicit port mappings exist for all edges.

    For unmapped edges, auto-generates canonical port mappings AND creates
    RefNode field expressions on canonical input fields so the wiring is
    visible (and editable) in the frontend.

    Returns a list of :class:`AutoGeneratedCanonicalExpression` for
    auto-generated canonical field expressions.  The caller must merge these
    into ``incoming_field_expressions_by_instance`` to prevent the cleanup
    pass from deleting them.
    """
    ...
    # Collect incoming field expression targets so we don't overwrite user edits.
    # NOTE: kind=INPUT parameters are extracted from instance.parameters earlier
    # in update_graph_service and passed here via input_params_by_instance.
    incoming_fe_targets: set[tuple[UUID, str]] = set()
    for inst in graph_project.component_instances:
        if inst.input_port_instances:
            for port_inst in inst.input_port_instances:
                if port_inst.field_expression and port_inst.field_expression.expression_json:
    ...
    if input_params_by_instance:
        for inst_id, params in input_params_by_instance.items():
            for param in params:
                if param.value:
                    incoming_fe_targets.add((inst_id, param.name))

    auto_generated_mappings: list[db.PortMapping] = []
    auto_generated_expressions: list[AutoGeneratedCanonicalExpression] = []
    ...
    for source_instance_id, target_instance_id in unmapped_edges:
        ...
        # Auto-populate a visible RefNode expression on the canonical input
        # so the user can see (and edit) the wiring in the frontend.
        if (target_instance_id, target_port_name) not in incoming_fe_targets:
            ...
            expression_json = {"type": "ref", "instance": str(source_instance_id), "port": source_port_name}
            auto_generated_expressions.append(
                AutoGeneratedCanonicalExpression(target_instance_id, target_port_name, expression_json)
            )
    ...
    return auto_generated_expressions
