...
async def update_graph_service(
    ...
    auto_generated_canonical_expressions = _ensure_port_mappings_for_edges(session, graph_runner_id, graph_project)
    ...
    for expr_info in auto_generated_canonical_expressions:
        ...
        if existing_port_id:
            ...
            expr = create_field_expression(session, expression_json)
            ipi = create_input_port_instance(
                session=session,
                component_instance_id=target_id,
                name=field_name,
                field_expression_id=expr.id,
            )
            db_port_instances_by_instance[target_id][field_name] = ipi.id
    ...
    auto_fe_schemas: list[AutoGeneratedFieldExpression] = []
    for expr_info in auto_generated_canonical_expressions:
        expression_json = expr_info.expression_json
        try:
    ...
    return GraphUpdateResponse(
        ...
        playground_input_schema=playground_input_schema,
        playground_field_types=playground_field_types,
        auto_generated_field_expressions=auto_fe_schemas,
    )
