...
# Import django-celery-beat models
import django
...
def create_or_update_crontab_schedule(
    cron_expression: str, 
    timezone: str = "UTC"
) -> CrontabSchedule:
    """
...
def ensure_cron_api_key_for_project(session: Session, project_id: UUID) -> Dict[str, Any]:
    ...
    return generate_cron_api_key_for_project(
        session=session,
...
def create_periodic_task_for_scheduler(
    ...
    try:
        ...
        if conversion_result["status"] != "SUCCESS":
            return {
                "status": "FAILED",
                "error": f"Invalid cron expression: {conversion_result['error']}"
            }
...
def create_schedules_for_graph(session: Session, project_id: UUID, graph_runner_id: UUID) -> Dict[str, Any]:
    ...
    if api_key_result["status"] not in ["SUCCESS", "EXISTS"]:
        error_msg = f"Failed to ensure cron API key for project {project_id}: {api_key_result.get('error', 'Unknown error')}"
        LOGGER.error(error_msg)
        return {
            "status": "FAILED",
            "error": error_msg,
            "schedules_created": 0,
            "api_key_error": api_key_result
        }
...
def cleanup_schedules_for_graph(session: Session, graph_runner_id: UUID, cleanup_api_key: bool = False) -> Dict[str, Any]:
    """
    ...
    if cleanup_api_key:
        try:
            # This is a simplified approach - in practice you'd need to get the project_id
            # from the graph_runner relationship
            api_key_cleanup_result = {"status": "SKIPPED", "reason": "Project ID not available"}
        except Exception as e:
...
def handle_scheduling_on_deployment(
    ...
    """
    LOGGER.info(
        f"Handling scheduling for deployment: project={project_id}, new_graph={graph_runner_id}, old_graph={previous_production_graph_id}"
    )
    ...
    schedulers = find_cron_scheduler_components(session, graph_runner_id)
    if not schedulers:
        LOGGER.info(f"No CRON_SCHEDULER components in new graph {graph_runner_id} - no scheduling needed")
        return {
            **results,
            "creation_results": {"status": "SKIPPED", "reason": "No CRON_SCHEDULER components", "schedules_created": 0}
        }
...
def cleanup_schedules_for_project(session: Session, project_id: UUID) -> Dict[str, Any]:
    """
