# Endpoint Polling Cron Job

## Overview

The `endpoint_polling` cron job monitors a GET endpoint and tracks new values that appear. When new values are detected, it can trigger workflows with the complete item data.
...
| `tracking_field_path` | string | No (default: "id") | Path to the tracking field in the response. Supports:<br>- `"id"` - Simple field<br>- `"data[].id"` - Array notation<br>- `"items[].id"` - Nested array |
...
1. **Extraction**: The cron job queries the endpoint and extracts all items using `tracking_field_path`
2. **Filtering**: If `filter_fields` is provided, only values matching all filter conditions are selected for tracking
3. **Tracking**: New values (not in the history) are saved to the database
4. **Workflow Triggering**: For each new value, if `project_id` is provided, a workflow is triggered with:
...
- The filtering (`filter_fields`) is only used for value selection, not for workflow input
- Complete items are always passed to workflows, regardless of filtering
- Values are tracked in the database; duplicates are prevented
- Only new values trigger workflows (values that weren't seen before)
