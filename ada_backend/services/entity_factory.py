...
def build_llm_service_processor(trace_manager: TraceManager, target_name: str = "llm_service") -> ParameterProcessor:
    def processor(params: dict, constructor_params: dict[str, Any]) -> dict:
        provider, model_name = get_llm_provider_and_model(params.pop("llm_model", "openai:gpt-4o-mini"))
        temperature = params.pop("llm_temperature", None)
        embedding_model_name = params.pop("embedding_model_name", None)
...
def build_qdrant_service_processor(trace_manager: TraceManager,
                                   target_name: str = "qdrant_service") -> ParameterProcessor:
    async def processor(params: dict, constructor_params: dict[str, Any]) -> dict:
        source_id_str = params.pop("data_source")["id"]
...
def compose_processors(*processors: ParameterProcessor) -> ParameterProcessor:
    async def composed(params: dict, constructor_params: dict[str, Any]) -> dict:
        result = params
