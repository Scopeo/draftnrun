...
def generate_cron_api_key_for_project(session: Session, project_id: UUID, creator_user_id: UUID) -> Dict[str, Any]:
    """
    ...
    try:
        api_key_response = generate_api_key(
            session=session, project_id=project_id, key_name=key_name, creator_user_id=creator_user_id
        )
...
def update_cron_api_key_for_project(session: Session, project_id: UUID, creator_user_id: UUID) -> Dict[str, Any]:
    """
    ...
    """
    results = {"status": "SUCCESS", "old_key_deactivated": False, "new_key_created": False, "error": None}

    # Deactivate existing cron API key if it exists
    existing_key = get_existing_cron_api_key(session, project_id)
    if existing_key:
        try:
            deactivate_api_key_service(session=session, key_id=existing_key.id, revoker_user_id=creator_user_id)
    ...
    try:
        new_key_result = generate_cron_api_key_for_project(
            session=session, project_id=project_id, creator_user_id=creator_user_id
        )
...
def cleanup_cron_api_keys_for_project(session: Session, project_id: UUID, revoker_user_id: UUID) -> Dict[str, Any]:
    """
    ...
    if not existing_key:
        return {"status": "SUCCESS", "message": "No cron API keys found to clean up", "keys_deactivated": 0}

    try:
        deactivate_api_key_service(session=session, key_id=existing_key.id, revoker_user_id=revoker_user_id)

        LOGGER.info(f"Cleaned up cron API key {existing_key.id} for project {project_id}")

        return {"status": "SUCCESS", "message": "Cron API key cleaned up successfully", "keys_deactivated": 1}

    except Exception as e:
        error_msg = f"Failed to cleanup cron API key: {str(e)}"
        LOGGER.error(error_msg, exc_info=True)
        return {"status": "FAILED", "error": error_msg, "keys_deactivated": 0}
