...
def _resolve_sections(
    ...
    for s in sections:
        ...
        if version.prompt_id != s.section_prompt_id:
            raise ValueError(f"Version {s.section_prompt_version_id} does not belong to prompt {s.section_prompt_id}")
...
def _verify_component_in_graph(session: Session, component_instance_id: UUID, graph_runner_id: UUID) -> None:
    ...
    if not node:
        raise NotFoundError(f"Component instance {component_instance_id} not found in graph {graph_runner_id}")
...
def pin_prompt_to_port_service(
    ...
    input_port_instance = get_input_port_instance(session, component_instance_id, port_name)
    if not input_port_instance:
        raise NotFoundError(f"Input port '{port_name}' not found on component instance {component_instance_id}")

    if not input_port_instance.port_definition or not input_port_instance.port_definition.is_prompt:
        raise PortNotPromptEligibleError(port_name, component_instance_id)
    ...
    input_port_instance.prompt_version_id = prompt_version_id

    literal_json = {"type": "literal", "value": version.content}
    if input_port_instance.field_expression:
        input_port_instance.field_expression.expression_json = literal_json
    else:
        fe = db.FieldExpression(expression_json=literal_json)
        session.add(fe)
        session.flush()
        input_port_instance.field_expression_id = fe.id
...
def get_project_prompt_pins_service(session: Session, project_id: UUID) -> list[PromptPinResponseSchema]:
    ...
    for ipi, pi, pv, prompt in pins:
        ci = session.query(db.ComponentInstance).filter(db.ComponentInstance.id == pi.component_instance_id).first()
