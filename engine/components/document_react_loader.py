...
class DocumentReactLoaderAgent(AIAgent):
    def __init__(
        self,
        llm_service: LLMService,
        trace_manager: TraceManager,
        component_attributes: ComponentAttributes,
        document_enhanced_llm_call: DocumentEnhancedLLMCallAgent,
        prompt: str = INITIAL_PROMPT,
        tool_description: ToolDescription = get_document_react_loader_tool_description(),
    ) -> None:
        self.agent_tools = [document_enhanced_llm_call]
        initial_prompt = self.get_initial_prompt(prompt, document_enhanced_llm_call)
        super().__init__(
            ...
            agent_tools=[document_enhanced_llm_call],
        )

    def get_initial_prompt(self, initial_prompt, document_enhanced_llm_call) -> str:
        return fill_prompt_template(
            initial_prompt,
            component_name="DocumentReactLoaderAgent",
            variables={"documents_tree": document_enhanced_llm_call.tree_of_documents},
        )
