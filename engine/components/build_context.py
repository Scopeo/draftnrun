...
from engine.components.types import SourceChunk, TermDefinition
...
def format_source_chunk_metadata(
    ...
    if llm_metadata_keys:
        metadata = {
            ...
            if key in metadata and key not in _excluded_keys
        }
    else:
        metadata = {
            key: value
            for key, value in metadata.items()
            if not key.startswith("_") and key not in _excluded_keys
        }
