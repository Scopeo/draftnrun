...
from engine.agent.data_structures import AgentPayload, ToolDescription, SplitPayload
...
DEFAULT_SPLITTER_TOOL_DESCRIPTION = ToolDescription(
    ...
    tool_properties={
        "delimiter": {
            "type": "string",
            "description": (
                "The delimiter to split content by (e.g., '\\n\\n' for paragraphs). "
                "If neither delimiter nor chunk_size is provided, defaults to '\\n\\n'"
            ),
        },
        "chunk_size": {"type": "integer", "description": "The maximum size of each chunk in characters"},
    },
...
class Splitter(Agent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(trace_manager, tool_description, component_instance_name, **kwargs)
    ...
    async def _run_without_io_trace(self, *inputs: AgentPayload, **kwargs) -> SplitPayload:
        ...
        # Split the content
        chunks = self._split_content(input_payload.content)
