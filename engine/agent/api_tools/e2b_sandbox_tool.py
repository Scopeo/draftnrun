    ...
    def execute_python_code(self, python_code: str, timeout: int = 60) -> dict:
        ...
        try:
            ...
            try:
                ...
                sandbox.filesystem.write(script_filename, python_code)
                
                # Execute the Python file
                result = sandbox.commands.run(
                    f"python3 {script_filename}",
                    timeout=timeout,
                )
                ...
                try:
                    sandbox.commands.run(f"rm -f {script_filename}", timeout=5)
                except Exception as e:
                    LOGGER.warning(f"Could not clean up script file: {str(e)}")
