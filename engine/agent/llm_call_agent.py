...
class LLMCallAgent(Agent):
    ...
    async def _run_without_io_trace(self, inputs: LLMCallInputs, ctx: dict) -> LLMCallOutputs:
        ...
        for prompt_var in prompt_vars:
            if prompt_var in ctx:
                ...
                raise ValueError(
                    ...
                    f"Available template vars: {list(ctx.keys())}"
                )

        # Handle file content from context
        if self._file_content_key and self._file_content_key in ctx:
        ...
        if self._file_url_key and self._file_url_key in ctx:
