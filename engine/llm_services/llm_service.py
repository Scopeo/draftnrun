...
class CompletionService(LLMService):
    ...
    async def _function_call_without_structured_output_async(
        ...
        formated_tools = [tool.openai_format for tool in tools]
        if not formated_tools:
            formated_tools = None
            tool_choice = None
    ...
    async def _function_call_with_structured_output_async(
        ...
        formated_tools = [tool.openai_format for tool in tools]
        if structured_output_tool is not None:
            formated_tools_with_structured = formated_tools + [structured_output_tool.openai_format]
        else:
            formated_tools_with_structured = formated_tools

        span = get_current_span()
        span.set_attributes({SpanAttributes.LLM_INVOCATION_PARAMETERS: json.dumps(self._invocation_parameters)})

        # When explicitly forced to avoid tools, use constrained completion path.
        if tool_choice == "none" or not formated_tools_with_structured:
        ...
        response = await client.chat.completions.create(
            model=self._model_name,
            messages=messages,
            tools=formated_tools_with_structured,
            temperature=self._invocation_parameters.get("temperature"),
            stream=stream,
            tool_choice=tool_choice,
        )
